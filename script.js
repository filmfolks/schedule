...
function handleFilterChange(e) {
    ...
    document.getElementById('filter-by-select').value = 'all'; // Reset dropdown visually
    if (filterType === 'all') {
        ...
        if (filterValue !== null && filterValue.trim() !== "") {
            ...
            activeFilter = { type: 'all', value: '' }; // Reset if user cancels
        }
    }
    renderSchedule();
}
...
function getVisibleScenes() {
    ...
    if (activeFilter.type === 'all') {
        return [...activeSequence.scenes]; // Return a copy
    }

    return activeSequence.scenes.filter(scene => {
...
function saveAsExcel() {
    ...
    const scenesToExport = getVisibleScenes(); // Use filtered scenes
    const sequenceName = activeSequence.name;
    if (scenesToExport.length === 0) { alert(`No visible scenes in "${sequenceName}" to export.`); return; }
    ...
    XLSX.utils.sheet_add_json(worksheet, scenesToExport, { origin: `A${header.length + 1}`, skipHeader: false });
